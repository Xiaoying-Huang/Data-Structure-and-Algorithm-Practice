...
function binarySearch(searchList, value, arrayPath = []) {
    if (searchList.length <= 1) {
        if (searchList[0] === value) {
            return [...arrayPath, value];
        } else {
            return 'Value Not Found';
        }
    } else {
        let dividePoint = Math.floor((searchList.length - 1)/ 2) ;
        arrayPath.push(searchList[dividePoint]);
        let newSearchList;
        if (value === searchList[dividePoint]) {
            return arrayPath;
        } else if (value > searchList[dividePoint]) {
            if (value < searchList[dividePoint + 1]) {
                return 'Value Not Found';
            } else {
                newSearchList = searchList.slice(dividePoint + 1);
            }
        } else {
            if (value > searchList[dividePoint - 1]) {
                return 'Value Not Found';
            } else {
                newSearchList = searchList.slice(0, dividePoint);
            }
        }
...
let a = binarySearch(testArray,70);
console.log(a);